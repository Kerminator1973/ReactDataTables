...
Решить эту проблему можно используя либо если вынести за пределы компонента, но это лишает нас возможности использовать несколько экземпляром компонента, либо применить Refs.

Мы можем создать Ref: `const timer = useRef();`
