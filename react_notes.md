...
В течение многих лет, наилучшим транспилятором считался [Babel](https://babeljs.io/), но сейчас часто используются [esbuild](https://esbuild.github.io/) и [SWC](https://swc.rs/docs/getting-started) - Speedy Web Compiler.
...
>- [Remix](https://remix.run/) избавляется от зависимостей конкретных Frameworks (т.е. уже не привязан к React)
>
...
>- Server Functions - Server Functions позволяют вызывать серверный код непосредственно из клиентского кода, как будто это обычная JavaScript-функция, без необходимости создавать отдельные API-эндпоинты. Фреймворк сам занимается передачей данных между клиентом и сервером. Примеры: React Server Actions (Next.js), tRPC, [Remix](https://remix.run/) от Shopify, SolidStart, SvelteKit
...
- [react-doctor](https://www.react.doctor/) - приложение для автоматического поиска ошибок в реализации React-приложений, включая Next.js и React Native
- [Oxidation Compiler](https://oxc.rs/) - набор инструментов для работы с JavaScript-кодом, написанных на Rust (быстрых): Linter, Formatter, Minifier, Transformer
...
Рекомендуется для ознакомления статья [React Compiler — конец эпохи useMemo?](https://habr.com/ru/sandbox/273490/). В статье рассматриваются случае, в которых компилятор не оптимизирует код:

- Мутация props — молчаливая смерть оптимизации
- Нестабильные ссылки из внешних библиотек
- Чтение Ref во время рендера
- Зависимость эффектов от идентичности объекта
- Некоторые популярные библиотеки несовместимы с React Compiler из-за нарушения Rules of React: React Hook Form, TanStack Table, MobX

По факту, конец **useMemo** не наступил - сбор метрик и ручная оптимизация всё ещё важны.
...
При указании необходимости _bundling_ компонентов, включая изображения, необходимо использовать относительный путь к картинкам в приложении. Например:
...
Однако следует иметь ввиду, что при включении изображении в Bundle, оно кодируется в base64, т.е. добавляет к размеру файла еще 33%. Это может существенно увеличить итоговый размер Bundle-а и значительно увеличить время старта приложения.

Перед принятием решения о размещении графического изображения в Bundle, подумайте - действительно ли это изображение необходимо поместить в bundle, или его вполне можно загружать из статической папки сервера и разрешить браузеру кэшировать его на локальном диске.
...
В примерах современного кода на React 19 часто используется `useImperativeHandle`, в частности, для **организации модальных диалогов**. Кажется, что императивный стиль программирования отыгрывает позиции, который он потерял в предыдущие годы.

## Полезные библиотеки

Для реализации бесконечных списков - [broad-infinite-list](https://github.com/suhaotian/broad-infinite-list)

## Изучение React

Очень полезный инструмент - сайт со статистическими материалами [State of React 2025](https://2025.stateofreact.com/ru-RU/). Информация на сайте позволяет ответить на такие вопросы: какие инструменты популярны? В каких направлениях развивается продукт? Какие языковые features востребованы?
