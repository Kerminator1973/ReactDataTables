...
// Определяем список свойств (props), которые должны быть переданы в модальный диалог
type EmployeeModalProps = {
    ...
    // При закрытии окна, обрабатывать это событие может родительский компонент
    onClose: () => void;
...
const EmployeeModal: React.FC<EmployeeModalProps> = ({
    ...
    // Хук useEffect() вызывается после того, как DOM будет изменён и
    // используется для проинициализации полей модального диалога,
    // в зависимости от его режима работы (новый/редактирование)
    useEffect(() => {
        if (isEditMode && initialData && show) {
            setFormData(initialData);
        } else {
            // Сбрасываем данные в форме, если она открывается в режиме добавления
            // нового сотрудника, или если модальное окно закрывается
