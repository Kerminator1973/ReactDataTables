...
## В работе Blazor с Bootstrap - тоже самое

Таблица с различиями в разных аспектах управления модальным диалогом в "Ванильном HTML + BS5" и "Blazor + BS5":

| Аспект                    | Ванильный HTML + BS5                      | Blazor                                                               |
|---------------------------|-------------------------------------------|----------------------------------------------------------------------|
| Управление видимостью     | JS-методы show()/hide() или data-атрибуты | Флаг IsVisible, привязанный к состоянию компонента                   |
| Передача данных в модалку | Заполнение DOM через JS перед открытием   | Параметры компонента и биндинг через @bind                           |
| Обработка событий         | Глобальные слушатели addEventListener     | Колбэки C#: EventCallback, @onclick                                  |
| Валидация                 | Ручная на JS                              | Встроенная через EditForm и аннотации данных                         |
| Переиспользование         | Копирование HTML или шаблонизаторы        | Компонент с RenderFragment — полноценная композиция                  |
| Сложность анимации        | Бесплатно из коробки (класс fade)         | Нужно реализовать самому (класс fade + Task.Delay для синхронизации) |

## Как развивается React Bootstrap
