...
>
>Кажется, что основная разница между JavaScript и TypeScript в масштабе проекта. Для простых проектов лучше подходит JavaScript - он проще. Для развивающихся проектов, которые могут вырасти в большие, кажется, что TypeScript - безальтернативен.

Очень важное свойство TypeScript состоит в удобстве миграции приложений с JavaScript на TypeScript. Не нужно переписывать все js-файлы на TypeScript сразу. Это можно делать постепенно, файл за файлом и приложение будет продолжать работать. Это оптимальный подход для крупных проектов. Одной из крупных компаний, которая использовала этот подход на практике является AirBNB.
...
## Функции высшего порядка (a higher-order function)

В мире TypeScript/JavaScript часто возникает потребность в создании функций, генерирующих другие функции. Примерами подобных потребностей являются:

- Мемоизация, техника оптимизации функциональных вызовов посредством кэширования результатов дорогостоющего вызова и использования этих результатов, когда они потребуются повторно
- Генерация обработчиков событий с предопределённым поведением. Например, обработчик, который пишет в лог сообщение о том, что событие возникло
- В Express.js функции высшего порядка часто используются при создании Middleware, в частности при создании ролевых моделей
- Currying, техника трансформации функции со множеством аргументов в последовательность функций, каждая из которых принимает только один параметр

Пример определения функции высшего порядка с использованием стрелочного синтаксиса:

```ts
// Определяем функцию высшего порядка, которая принимает на входе некоторое значение и
// возвращает функцию, которая будет суммировать это значение с переданным в
// сгенерированную функцию
const createAdder = (base: number) => (addend: number): number => {
    return base + addend;
};
...
console.log(addFive(10));   // Выводит: 15

const addTen = createAdder(10);
console.log(addTen(3));     // Выводит: 13
console.log(addTen(10));    // Выводит: 20
```

Приведённая выше функция высшего порядка может быть реализована и с более традиционным синтаксисом:
...
В Angular декоратор `@Component()` может применяться только к классу, а декоратор `@Input()` только к свойствам класса.

Декораторы активно используются в Next.js, MobX и Stencil.js.

Использование декораторов более лаконичное, читаемое и декларативное решение, чем наследование классов.

В комитете по стандартизации рассматривается добавление декораторов в JavaScript.
