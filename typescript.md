...
Миксин (русский перевод "примесь") - это способ композиции поведения (методы и свойства) в классы с целью их повторного использования, но без использования наследования от одного базового класса.

Миксины позволяют избегать глубоких деревьев наследования (deep inheritance trees) и позволяют сделать "горизонтальный" код повторно используемым.

Миксин получает на входе некоторый класс и создаёт новый класс, добавляя в оригинальный класс дополнительные методы и функции. Таким образом реализуется расширенное поведение. "Под капотом" миксины используют декораторы.

В реальных приложения миксины чаще всего используется для добавления в класс следующих функций: логирование, генерация событий, делегирование, эмуляция множествевнного наследования.
