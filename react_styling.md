...
Добавление Tailwind в React-проект описана в [отдельном документе](./react_tailwind.md)
...
>Среди Frontend-разработчиков отношение к Tailwind не однозначное из-за его многословности. В некоторых случаях, классы DOM-элемента занимают две-три экранные строки и их очень тяжело вдумчиво читать. С учётом распространения генерации кода ИИ, проблема стала критичной, т.к. ИИ беспощадно генерирует очень развесистые классы.
>
>В качестве альтернативы рассматриваются [Mantine UI](https://mantine.dev/) by Vitaly Rtishchev и [Chakra UI](https://chakra-ui.com/)
