...
## Передача параметров в компонент с помощью spread operator
...
Мы можем передать JSX-компонент в качестве параметра - в следующем ниже примере это осуществляется через параметр **Icon**:
...
Однако вызвать функцию формирования - `Icon()` для встраивания параметра в цепочку rendering-а.
...
## Генерация верстки для массива, в который вложен другой массив

Предположим, что нам нужно вывести таблице 3 на 3, которая определена в переменной initialGameBoard:
...
Как и в случае с одномерными массивами, мы можем использовать метод **map**(), но поскольку массив является вложенным, то мы будем использовать один вложенный map(), внутри другого:
