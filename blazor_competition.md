...
>Дополнительное преимущество Blazor - может быть использован в качестве front-а в Maui - в том числе, для разработки мобильных приложениний. Maui предоставляет разработчикам мобильных приложений выбор технологии для разработки frontend: XAML, или Blazor.
>
>К сожалению, без проблем решение Maui/Blazor обойтись не смогло. На мобильных устройствах с Android для формирования пользовательского интерфейса используется WebView, который не поддерживает российские сертификаты Минцифры. После введения санкций и блокировки сертификатов выдаваемым российским компаниям, возник значимый риск того, что все решения на WebView для крупных российских компаний будут блокироваться на этапе установки соединения с Backend.
>
>Также важно заметить, что обеспечение Platform-Specific - существенная задача, т.к. каждая платформа имеет свои существенные особенности: мобильные приложения - жесты пальцами (touch gestures), Desktop - точные движения мышью, Web - ограничения браузера.

Важно отметить, что экосистема .NET сфокусирована на предотвращении дублирования кода. Например, использование Entity Framework Code First позволяет описать модель один раз и автоматически генерировать схему базы данных с помощью миграций. Тоже самое можно сказать и о обмене сообщениями по сети между клиентом и браузером: в Blazor-проекте достаточно создать Shared-библиотеку, в которой будут описаны Requests и Responses, а затем использовать эту библиотеку и в клиенте, и в сервере. Если мы используем, например, React и Spring Boot, то потребуется описать запросы и ответы как на Java, так и на JavaScript. Это не только увеличивает объём работы вдвое, но и требует синхронизации кода в разных проектах при изменении протокола взаимодействия! Заметим, что повторно использовать можно не только запросы/ответы, но и:

- модели
- сервисы
- разделяемы компоненты (например, один и тот же компонент может использовать в Blazor-клиенте, и в серверном приложении)
- стили

## Оценки производительности Wasm в браузере

Рекомендуется для ознакомления статья [WebAssembly: обзор технологии, эксперименты с числами и мечты о браузере без JS](https://habr.com/ru/companies/psb/articles/1028368/) by TheMadLord.

По материалам статьи, говорить о преимуществве Wasm над JavaScript в части организации пользовательского интерфейса, в 2026 году не приходится. Всё основное взаимодействие с DOM, файловой системой, сервисами операционной системы Wasm осуществляет через JavaScript-мост. Т.е. производительности подобных задач упирается в JS.

Также из-за изоляции модулей Wasm, взаимодействие между ними также строиться через JS.

Преимущество в скорости может быть достигнуто в отдельных вычислительных задачах, в минимальной степени использующих ресурсы браузера. На основании выводом статьи:

- операции со строками - Wasm сильно проигрывает JS
- математические вычисления - Wasm показывает лучшие результаты, чем JS на 20-25%
- криптография - кратно лучшая производительность у Wasm

Таким образом, если мы говорим о Wasm 2.0, но преимущества в производительности достижимы лишь в специфических задачах. Отличная многолетняя оптимизация JS Runtime играет не в пользу Wasm.

Скорее, Wasm сейчас это не про производительность, а про перенос чужих экосистем (в частности .NET, C++ и Rust) в браузер. Это больше про созданию унифицированных инстументальных средств, а не про эффективность управления вычислительными ресурсами.
...
В условиях лавинообразного развития ИИ, новые разработчики, задавая вопрос ChatGPT, какой фреймворк следует использовать, будут гарантированно получать рекомендацию - React. Точно такой же результат будут получать программисты, которые интересуются размером community. Это приведёт к тому, что cо временем, доминирование React станет близким к 100%.
...
Синтаксис Blazor исключительно близок к синтакису Razor Pages в ASP.NET Core, что означает низкий порог вхождения ASP.NET Core разработчиков в Blazor.

Ещё один субъективный фактор: Microsoft использует React в промышленных масштабах, включая пользовательский интерфейс Microsoft Windows.

В браузере Edge React применялся для создания отдельных элементов пользовательского интерфейса. Например, меню, выпадающие списки и вкладка "Избранное" представляли собой мини-приложения на React. Однако в 2024 году Microsoft объявила о переходе на новую архитектуру WebUI 2.0, отказавшись от дальнейшего использования React в Edge. Причинами стали проблемы с производительностью и модульностью кода:

- Проблемы с модульностью. Команды, работавшие над разными компонентами, использовали общие пакеты и файлы, что приводило к замедлению работы интерфейса, так как одна часть UI замедляла другую из-за ненужных зависимостей
- Использование клиентского рендеринга на JavaScript. Microsoft считала, что это замедляло работу браузера
- Некорректное использование React. Компания часто встраивала HTML-страницы с React в браузер для управления интерфейсом, что приводило к избыточному использованию JavaScript

Fluent UI React — это библиотека React-компонентов и система дизайна, разработанная Microsoft.

Microsoft разрабатывала React Native для Windows — фреймворк для создания нативных приложений для Windows с использованием React.

Таким образомб субъективно, инженеры Microsoft должны очень хорошо понимать ограничения React, что, в свою очередь, могло бы быть использовано в качестве требований к Blazor.
...
- Для того, чтобы избежать сцепления кода из-за Prop Drilling, в React используется Context API (State Management). В Blazor тоже самое делается через CascadingParameters и вся сложность механизма скрывается "под капотом". В этом есть как удобство Blazor (сложность скрывается), так и ограничение (сложно оценить эффективность работы этого механизма и побочки)
- Blazor-приложение не может быть запущено с локального диска как статическое web-приложение. React-приложение может быть запущено с локального диска, но у него не будут работать: 
    - маршрутизация (React Router)
    - API‑запросы
    - динамическая загрузка ресурсов
    - хеш‑роутинг

>Как обычно, как только начинаешь исследовать что-то достаточно глубоко, то появляется много новых деталей и всё становится не так однозначно. Что касается CascadingParameters, то:
>
>Альтернативы:
>
>- Сервисы с DI. Зарегистрируйте класс состояния как сервис (Scoped/Singleton) и внедряйте его через @inject. Подходит для сложной логики и разделения ответственности
>- EventAggregator. Реализуйте шину событий для коммуникации между компонентами
>- LocalStorage/SessionStorage. Для сохранения состояния между сессиями (например, тема интерфейса)
>
>Рекомендации по использованию:
>
>- CascadingValue/Parameter — для UI‑состояния (тема, локализация), которое нужно многим компонентам
>- Сервисы — для бизнес‑логики, работы с API, кэширования
>- EventAggregator — для слабосвязанных компонентов, не находящихся в одной иерархии
>
>Т.е. кажется, что на React's Context API больше похож EventAggregator, а не CascadingValue/Parameter
...
- изменит переменную в области React, хранящую tableData
- неявным образом вызовет re-render
