...
Т.е. он, вероятно, "сбрасывается" родительским элементом через injection:

```csharp
@inject IDialogService DialogService
```

Но это не очевидно, т.к. нужно знать, сложно вывести логически. Наверное, это самом слабое место Blazor.
