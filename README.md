...
Использование модели SPA даёт целый ряд преимуществ в разработке пользовательского интерфейса, к которым относятся:

- декларативный стиль программирования
- развитая компонентная модель
- возможность кэширования артефактов на уровне приложения. Например, ресурс загруженный на одной "странице", может быть использован и на других "страницах"
- приложение сохраняет работоспособность даже при временной недоступности сервера, с обеспечением возможности продолжения работы без потери ранее полученных данных

Если рассматривать принципиальные причины по которым SPA с компонентной моделью позволяет значительно упростить разработку кода, то следует отметить два наиболее важных аспекта:

- Компонентная модель позволяет разделить очень сложное **состояние** страницы, а несколько гораздо более простых **состояний** компонента. Состояние - это набор данных, которые непосредственно влияют на внешний вид и поведение пользовательского интерфейса
- При обработке событий, изменяется только **состояние**, т.е. данные (у которых должна быть чёткая модель), тогда как внешний вид формируется движком (Blazor, React), который использует **состояние** и декларативный код, описывающий пользовательский интерфейс

>На практике может быть очень важно кэширование данных, например, прав пользователя. Например, в одном из MDI-приложений, разработанных на ASP.NET Core 8 с Razor Pages, на каждой странице 14 раз проверялись права доступа к элементами меню. Выглядело это приблизительно так:
>
>```csharp
>@if ((await AuthorizationService.AuthorizeAsync(User, "ReportReminders", RoleRequirements.Read)).Succeeded)
>{<a class="dropdown-item" asp-page="/ReportReminders">Текущие остатки</a>}
>```
>
>Также права запрашивались и для блокировки/разблокировки отдельных кнопок. Таким образом, при генерации каждой страницы выполнялось порядка 20 запросов к СУБД, связанных только с авторизацией операции.

## Генерация кода базового приложения
...
## В чём основной плюс декларативного программирования

Основное преимущество декларативного программирования заключается в том, что оно позволяет **сосредоточиться на описании желаемого результата**, а не на деталях его достижения.

- Императивный стиль: подробно описываются все шаги и порядок их выполнения
- Декларативный стиль: задаётся условие результата, а система сама определяет оптимальный способ выполнения

Реальный негативный пример в котором отказ от декларативного стиля в пользу императивного ухудшил качество кода:

Было (декларативный стиль):

```csharp
<SanitizedInput Class="mb-3 w-100 form-control"
                InputValue="@BindValue"
                ValueChanged="@OnValueChanged"
                InputRegex="@("[^А-Яа-яA-Za-z0-9-_!?,.@ ]")"
                Placeholder="Ф.И.О."
                Required
                Style="height:50px"
                Type="text"
...
<script>
    document.getElementById('nameInput')?.addEventListener('input', function(e) {
        ...
        var filtered = value.replace(/[^A-Za-zА-Яа-яЁё ]/g, '');
        if (value !== filtered) {
            e.target.value = filtered;
            // Триггерим событие для Blazor
...
Если не затрагивать возникновение лишнего каскада в обработке сообщений (сначала событие input обрабатывается в JavaScript, а потом пробрасывается в Blazor Wasm), то основная проблема - это жесткая привязка к конкретному органу управления по id (`nameInput`). Т.е. был компонент, который можно было многократно использовать на странице, а его заменили на DOM-элемент с привязанным JavaScript-кодом, который будет требовать добавления императивного кода каждый раз, когда потребуется добавить на страницу ещё один такой элемент.

### Декларативный стиль очень хорош для форм со сложным поведением

Ещё один практический пример - разработка формы проверки серийных номеров через RUFServer. Пользовательский интерфейс системы выглядит следующим образом:

![Проверка серийных номеров](./serial_number.png)

Особенность пользовательского интерфейса состоит в том, что в форме динамически изменяемое количество блоков для указания нескольких серийных номеров, а состав полей в выпадающем списке модификаций прибора зависит от модели прибора, которая может быть изменена в произвольный момент времени. Если бы мы реализовывали этот функционал в императивном стиле, добиться высокой надёжности кода было бы сложно. В декларативном стиле верстка отделена от данных и программисту, чаще всего достаточного того, чтобы в одном источнике данные были корректны.

Посмотреть реализацию можно в проекте SPCD, в компоненте "ServicePartners.Client\Components\DorsService\ModalForm\RequestModalForm.razor"

## Компонентная модель - изоляция состояния

Благодаря чему использование компонентной модели приводит к более надёжному коду? Если рассмотривать создание сложных форм, можно заметить, что их состояние чаще всего можно упростить разбив на несколько значительно более простых состояний. Предположим, что нам нужно создать модальное окно для отправки заявки на обновление счётчика купюр. В реальной заявке ГК ДОРС есть поля относящиеся к данным пользователя (имя, телефон и почта), блок для проверки возможности обновления счётчика по его серийному номеру, блок загрузки прикреплямых файлов, captcha. В реальном приложении, блок проверки возможности обновления обладает сложной динамической структурой, но для всей формы важными являются только результат валидации и серийные номера приборов. Этот блок - идеальный кандидат на самостоятельный компонент. Всей форме не важно, как именно осуществляется проверка серийных номеров, проверяется ли группа, или только один номер - важен только результат. Соответственно, отдельный компонент позволяет скрыть своё собственное состояние от родительского компонента и таким образом значительно упростить сложность родителя.

Возможность изоляции внутренних состояний компонентов - одна из основных причин создавать новые компоненты и, принципиально, использовать компонентную модель.

## Подключение DataTables.NET

Цель данного исследования - убедиться в том, что можно использовать одну из лучших библиотек, функционировавших в императивном стиле программирования, в приложении на React со 100% декларативным стилем разработки приложений.
